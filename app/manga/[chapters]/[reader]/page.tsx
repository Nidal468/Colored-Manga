...
export default function Reader(params: any) {
    ...
    const [fit, isFit] = useState(false)
        ...
        <div className='w-full flex items-start justify-start'>
            <div className="font-light flex flex-col items-center justify-start gap-[10vw]" style={{ width: divStates.div4 ? "80%" : "100%" }}>
                ...
                {divStates.div4 ? <Sidebar
                    ...
                    fitWidth={() => { setWidth("95%"), setHeight("100%"), isFit(false) }}
                    fitHeight={() => { setHeight("100%"); setWidth("37%"), isFit(true) }}
                    fitRes={() => { setWidth("75%"), setHeight("100%"), isFit(false) }}
                /> : <></>
                }
                ...
                <div className='w-[95%] lg:w-full flex flex-col items-center justify-start gap-[1vw] lg:mb-[50px] mb-[5vw]' style={{ marginTop: divStates.div5 ? "0px" : "50px" }}>
                    {selectedChapter?.images?.map((image: any) => (
                        <>
                            {fit ? <div style={{ height: '100vh', width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                <img
                                    style={{ height: '100%', width: 'auto' }}
                                    src={image.source}
                                    alt={image.source}
                                />
                            </div> : <div className='relative z-0' style={{ width: width, height: height }}><Image
                                width={4000}
                                height={4000}
                                src={image.source}
                                alt={image.source}
